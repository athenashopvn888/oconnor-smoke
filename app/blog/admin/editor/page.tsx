...
function BlogEditorInner() {
  ...
  const [author, setAuthor] = useState("OCONNOR SMOKE Team");
  ...
  useEffect(() => {
    ...
    fetch(`${APPS_SCRIPT_URL}?action=blog&store=OSC01&admin=1`)
      .then((r) => r.json())
      .then((data) => {
        ...
        if (post) {
          ...
          setAuthor(post.author || "OCONNOR SMOKE Team");
  ...
  async function handleSave() {
    ...
    const payload = editId
      ...
      : { action: "create", title, content, author, published, store: "OSC01" };
